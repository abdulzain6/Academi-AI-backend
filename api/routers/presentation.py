...
from ..globals import collection_manager, file_manager, presentation_maker, template_manager
...
def verify_file_existance(user_id: str, file_names: list[str], collection_uid: str) -> bool:
    return all(file_manager.file_exists(user_id, collection_uid, file_name) for file_name in file_names)
...
def make_presentation(presentation_input: MakePresentationInput, user_id=Depends(get_user_id)):
    if presentation_input.use_data:
        if presentation_input.collection_name:
            if collection := collection_manager.get_collection_by_name_and_user(presentation_input.collection_name, user_id):    
                vectordb_collection = collection.vectordb_collection_name
            else:
                raise HTTPException(400, "Collection does not exist.")

            if presentation_input.files and not verify_file_existance(user_id, presentation_input.files, collection.collection_uid):
    ...
    return FileResponse(file_path, headers={"Content-Disposition": f"attachment; filename={file_path.split('/')[-1]}"})
