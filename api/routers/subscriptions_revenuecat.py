...
def subscription_usage(purchased_at_ms: int, expiration_at_ms: int, event_timestamp_ms: int) -> float:
    logging.info(f"Calculating subscription usage: purchased_at_ms={purchased_at_ms}, expiration_at_ms={expiration_at_ms}, event_timestamp_ms={event_timestamp_ms}")
    total_duration = expiration_at_ms - purchased_at_ms
    elapsed_time = event_timestamp_ms - purchased_at_ms
    if total_duration == 0:
        return 0
    logging.info(f"Calculating subscription usage: total_duration={total_duration}, elapsed_time={elapsed_time}")
    usage = min(max((elapsed_time / total_duration) * 100, 0), 100) / 100 # Clamp between 0-100
...
def handle_subscription_purchase(user_id: str, purchase_token: str, product_id: str, purchase_ms: float, expire_ms: float):
    subscription_manager.add_subscription_token(user_id, purchase_token)
    logging.info(f"Subscription purchase/renewal attempt by {user_id} with token {purchase_token}")

    multiplier = get_multiplier(product_id)
    if product_id not in PRODUCT_ID_MAP_REVENUE_CAT:
        logging.error(f"Product not found for User: {user_id} with Product ID: {product_id}")
        return
    ...
    logging.info(f"{user_id} subscribed with token {purchase_token} for product {product_id}")
    ...
    if product_id in PRODUCT_ID_COIN_MAP:
        ...
        if event_type == "NON_RENEWING_PURCHASE":
            if transaction_id in subscription_manager.retrieve_onetime_tokens(user_id):
                logging.error(f"Coins purchase attempt by {user_id} with transaction ID: {transaction_id}. Tokens already used.")
                logging.info(f"User coins: {user_points_manager.get_user_points(user_id)}")
                return
            ...
            logging.info(f"Coins purchase granted for {user_id}. Transaction ID: {transaction_id}.")
            ...
            logging.info(f"{user_id} had a refund. Coins decremented: {coins_to_decrement}.")
        ...
        if event_type == "INITIAL_PURCHASE" or event_type == "RENEWAL":
            handle_subscription_purchase(user_id, transaction_id, product_id, purchase_ms=purchased_at_ms, expire_ms=expiration_at_ms)

        elif event_type == "EXPIRATION":
            user_sub = subscription_manager.fetch_or_cache_subscription(user_id=user_id)
            logging.info(f"Subscription expired for {user_id} with transaction ID: {transaction_id}")
            if period_type == "TRIAL":
                coins = SUB_COIN_MAP_REVENUE_CAT[product_id]
                coins_to_decrement = coins * get_multiplier(product_id)
                user_points_manager.decrement_user_points(user_id, int(coins_to_decrement))
                logging.info(f"{user_id} trial expired. Coins decremented: {coins_to_decrement}")
            ...
            if reason == "CUSTOMER_SUPPORT":
                ...
                if user_sub.get("purchase_token") == transaction_id:
                    ...
                    logging.info(f"{user_id} coins decremented due to customer support cancellation. Coins: {coins}")
            else:
            ...
            user_points_manager.decrement_user_points(user_id, int(coins_to_decrement))
            logging.info(f"{user_id} coins decremented due to product change. Coins: {coins_to_decrement}")
            ...
            subscription_manager.reset_monthly_limits(user_id, get_multiplier(new_product_id))
            logging.info(f"Subscription updated for {user_id} due to product change. New product ID: {new_product_id}")

        else:
            logging.info(f"Unhandled Event for {user_id}: {event}")
    else:
        logging.info(f"Unhandled product ID for user {user_id}: {product_id}, Event: {event}")
