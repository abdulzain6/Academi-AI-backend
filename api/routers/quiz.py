...
def make_quiz(
    ...
    model = get_model({"temperature": 0}, False, premium_model, alt=False, json_mode=True)    
    ...
    try:
        ...
        questions = quiz_generator.generate_quiz(
            select_random_chunks(data, 1000, 1000),
...
def evaluate_quiz(
    ...
    model = get_model({"temperature": 0}, False, False, alt=False, json_mode=True)
    quiz_generator = QuizGenerator(
        file_manager,
        None,
        model
